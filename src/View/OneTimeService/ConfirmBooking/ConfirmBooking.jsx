  ...
  const paymentFunc = async () => {
    try {
      ...
      const res = await initatePayment({
        ...
        success_url: `http://localhost:5173/confirm-booking/${id}/${coachId}/?slot=${encodedSlot}&status=success`,
        cancel_url: `http://localhost:5173/confirm-booking/${id}/${coachId}?status=cancel`
      });
      if (res?.success) {
